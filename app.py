...
# النظام الداخلي
        ...
        if menu == "🏠 لوحة التحكم":
            ...
            if user_plans:
                ...
                for i, plan in enumerate(user_plans):
                    done = int(plan.get('completed', 0) if plan.get('completed') else 0)
            ...
            if user_plans:
                ...
                if target_plan:
                    ...
                    old_count = int(target_plan.get('completed', 0) if target_plan.get('completed') else 0)
    ...
    # ==========================
    # واجهة المدير (المطورة جداً)
    # ==========================
    elif st.session_state.role == "admin":
        
        # 1. لوحة القيادة (Dashboard): إحصائيات عامة
        ...
        # 2. إدارة المحاربين: الجدول والحذف
            ...
            # استبعاد الأدمن من قائمة الحذف
            student_list = [u['username'] for u in users if u.get('role') != 'admin']
            
            if student_list:
                c1, c2 = st.columns([3, 1])
                with c1: 
                    target_user = st.selectbox("اختر محارباً لاستبعاده نهائياً:", student_list)
                with c2: 
                    st.write("") # مسافة للتنسيق
                    st.write("") 
                    if st.button("🔴 تنفيذ قرار الطرد", use_container_width=True):
                        if delete_user_cloud(target_user):
                            st.success(f"تم استبعاد {target_user} من النظام بنجاح.")
                            st.rerun()
            else:
                st.info("لا يوجد طلاب لحذفهم حالياً.")

        # 3. مراقبة العمليات: رؤية ماذا يدرس الطلاب (الميزة الجديدة)
